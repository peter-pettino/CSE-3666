...
dot_product:
        ...
        # integer 0 to SP 0
        fcvt.s.w ft0, t0	# ft0 = 0.0
        fcvt.s.w fs0, t0	# fs0 = 0.0
loop:
	bge	t0, a2, f_exit	# if(i >= n) goto f_exit
	
	slli	t1, t0, 2	# t1 = i * 4
	add	t2, t1, a0	# t2 = x[i]
	add	t3, t1, a1	# t3 = y[i]

	flw	ft1, 0(t2)	# load value at x[i] to ft1
	flw	ft2, 0(t3)	# load value at y[i] to ft2
	
	fmul.s	ft3, ft1, ft2
	fadd.s	fs0, fs0, ft3	# sum += x[i] * y[i]
	
	addi	t0, t0, 1	# i += 1
	beq	x0, x0, loop	# goto loop
f_exit:
	fadd.s	fa0, ft0, fs0	# set return value to fs0
	jalr	x0, ra, 0	# return
